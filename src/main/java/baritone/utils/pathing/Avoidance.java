    ...
    public static List<Avoidance> create(IPlayerContext ctx) {
        ...
        if (mobCoeff != 1.0D) {
            ctx.world().loadedEntityList.stream()
                    // Filter only EntityMob instances or EntityTNTPrimed
                    .filter(entity -> entity instanceof EntityMob || entity instanceof EntityTNTPrimed)
                    // Exclude EntitySpider unless player brightness is less than 0.5
                    .filter(entity -> !(entity instanceof EntitySpider) || ctx.player().getBrightness() < 0.5)
                    // Include EntityPigZombie if angry, and EntityEnderman if screaming
                    .filter(entity ->
                            !(entity instanceof EntityPigZombie) || ((EntityPigZombie) entity).isAngry() ||
                                    !(entity instanceof EntityEnderman) || ((EntityEnderman) entity).isScreaming()
                    )
                    // For each filtered entity, create an Avoidance object and add it to the res list
